...
#define SET_MARK(x, y, s) M(x, y) = s & 2 ? s & 1 ? M(x, y) | 12 : (M(x, y) & 243) | 8 : s & 1 ? M(x, y) & 243 | 4 \
                                                                                               : M(x, y) & 243
#define INC_NUM(x, y)  \
    if (!IS_OUT(x, y)) \
    M(x, y) = (M(x, y) & 15) | (GET_NUM(x, y) + 1) << 4
...
void print(int c)
{
    ...
    for (y = col - 1; y >= 0; --y)
    {
        ...
        if (c)
            ...
            for (x = 0; x < len; ++x)
                printf("%-2c", IS_VISI(x, y) ? IS_MINE(x, y) ? '*' : GET_NUM(x, y) + '0' : p[GET_MARK(x, y)]);
        printf("%-2d", y);
        printf("\n");
    }
    printf("  ");
